...
Run the [WebSocket verification steps](client-setup.md#verify-websocket-transport). If verification passes but the CLI still falls back, restart the CLI so it re-reads the transport setting (a one-time step after changing transport config). If codex-lb sits behind a reverse proxy, make sure it forwards WebSocket upgrades — see [Remote Access](deployment/remote.md).
