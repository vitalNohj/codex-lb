...
class AccountsService:
    ...
    async def list_accounts(self, *, account_ids: list[str] | None = None) -> list[AccountSummary]:
        ...
        if account_ids is None:
            ...
            opencode_go_synthetic = await self._opencode_go_sidecar_account_summary()
            if opencode_go_synthetic is not None:
                summaries.append(opencode_go_synthetic)
    ...
    async def _opencode_go_sidecar_account_summary(self) -> AccountSummary | None:
        if self._settings_repo is None:
            return None
        settings = await self._settings_repo.get_or_create()
