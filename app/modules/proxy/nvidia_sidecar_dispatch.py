...
NVIDIA_SIDECAR_SOURCE = "nvidia_sidecar"
...
class NVIDIAChatPayload:
    body: dict[str, JsonValue]
    requested_reasoning_effort: str | None = None
    effective_reasoning_effort: str | None = None


def nvidia_routing_entry(config: NvidiaSidecarConfig) -> SidecarRoutingEntry:
...
async def load_nvidia_sidecar_config() -> NvidiaSidecarConfig | None:
    try:
        dashboard_settings = await get_settings_cache().get()
    except Exception:
        logger.warning("failed to load dashboard settings for NVIDIA sidecar", exc_info=True)
        return None
    return nvidia_sidecar_config_from_settings(dashboard_settings)


def nvidia_sidecar_config_from_settings(settings: DashboardSettings) -> NvidiaSidecarConfig:
    api_key = _decrypt_nvidia_secret(settings.nvidia_sidecar_api_key_encrypted)
    return NvidiaSidecarConfig(
        enabled=bool(settings.nvidia_sidecar_enabled),
        base_url=settings.nvidia_sidecar_base_url.rstrip("/"),
        api_key=api_key,
        prefixes=parse_sidecar_prefixes(settings.nvidia_sidecar_model_prefixes_json),
        connect_timeout_seconds=settings.nvidia_sidecar_connect_timeout_seconds,
        request_timeout_seconds=settings.nvidia_sidecar_request_timeout_seconds,
        models_cache_ttl_seconds=settings.nvidia_sidecar_models_cache_ttl_seconds,
        full_models=parse_sidecar_full_models(settings.nvidia_sidecar_full_models_json),
        default_reasoning_effort=settings.nvidia_sidecar_default_reasoning_effort,
    )


def _decrypt_nvidia_secret(encrypted: bytes | None) -> str | None:
    if not encrypted:
        return None
    try:
        return TokenEncryptor().decrypt(encrypted)
    except Exception:
        logger.warning("failed to decrypt NVIDIA sidecar API key", exc_info=True)
        return None


def build_nvidia_chat_payload(
...
async def proxy_chat_to_nvidia(
    request: Request,
    payload: ChatCompletionsRequest,
    *,
    effective_model: str,
    ...
    client: NvidiaSidecarClient,
    cursor_compat: bool = False,
    wire_model: str | None = None,
) -> Response:
    sidecar_payload = build_nvidia_chat_payload(payload, wire_model or effective_model, client.config)
    ...
    if payload.stream:
        ...
        stream: AsyncIterator[bytes] = _nvidia_stream_iterator(
    ...
    try:
        ...
        settlement = await external_response_settlement(
            provider=NVIDIA_PRICING_PROVIDER,
            model=effective_model,
    ...
    cost = await _nvidia_request_cost(effective_model, usage, billed_cost_usd=billed_cost_usd)
    await _finalize_or_release_nvidia_reservation(
        reservation,
        api_key=api_key,
        model=effective_model,
...
class _SseUsageDecoder:
    ...
    NIM is one OpenAI-compatible server among many an operator can point this
    sidecar at, so neither property below is theoretical. Both mirror the
    decoder in ``app/modules/proxy/opencode_go_sidecar_dispatch.py``.
...
async def _nvidia_request_cost(
    model: str,
    usage: SidecarUsage | None,
    *,
...
async def _log_nvidia_request(
    *,
    ...
    try:
        if cost is None:
            cost = await _nvidia_request_cost(model, usage)
        ...
        logger.warning(
            "failed to write NVIDIA sidecar request log key_id=%s request_id=%s",
...
async def _finalize_or_release_nvidia_reservation(
    reservation: ApiKeyUsageReservationData | None,
    *,
    api_key: ApiKeyData | None,
    model: str,
    ...
    try:
        ...
        logger.warning(
            "failed to settle NVIDIA sidecar API key reservation key_id=%s request_id=%s",
            api_key.id if api_key else None,
            get_request_id(),
            exc_info=True,
        )
...
async def _release_nvidia_reservation(
    reservation: ApiKeyUsageReservationData | None,
    *,
    api_key: ApiKeyData | None,
) -> None:
