...
class NvidiaSidecarError(Exception):
    def __init__(
        self,
        status_code: int,
        message: str,
...
class NvidiaSidecarUnavailableError(NvidiaSidecarError):
...
NVIDIA_PRICING_PROVIDER = "nvidia"


class NvidiaSidecarClient:
    ...
    @property
    def config(self) -> NvidiaSidecarConfig:
        return self._config
    ...
    async def list_models(self) -> list[SidecarModel]:
        ...
        except NvidiaSidecarError:
            raise
        except (asyncio.TimeoutError, aiohttp.ClientError, OSError) as exc:
            raise NvidiaSidecarUnavailableError(_transport_message(exc, "fetch NVIDIA sidecar models")) from exc

        if not is_json_mapping(data):
            raise NvidiaSidecarError(502, "Invalid response format from NVIDIA models API", body=data)
        raw_models = data.get("data")
        if not isinstance(raw_models, list):
            raise NvidiaSidecarError(502, "Missing 'data' key in NVIDIA models response", body=data)
        ...
        for entry in raw_models:
            ...
            models.append(
                SidecarModel(
                    id=model_id,
                    created=created_at,
                    owned_by=owned_by if isinstance(owned_by, str) else "nvidia",
                    raw=cast(Mapping[str, JsonValue], entry),
                    pricing=_parse_nvidia_pricing(entry.get("pricing")),
                )
            )
    ...
    async def list_models_cached(self) -> list[SidecarModel]:
        ...
        try:
            ...
            if self._models_cache is not None:
                logger.warning("using cached NVIDIA sidecar models after refresh failure", exc_info=True)
                return list(self._models_cache)
            logger.warning("NVIDIA sidecar models unavailable", exc_info=True)
            return []
    ...
    async def chat_completion(self, payload: Mapping[str, JsonValue]) -> JsonValue:
        ...
        try:
            ...
            raise NvidiaSidecarUnavailableError(_transport_message(exc, "call NVIDIA sidecar")) from exc
...
def _error_from_status(
    ...
    message = f"NVIDIA sidecar returned HTTP {status_code}"
    ...
    return NvidiaSidecarError(status_code, message, body=body, headers=_rate_limit_headers(headers))
