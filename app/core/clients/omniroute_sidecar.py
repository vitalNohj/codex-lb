...
class OmniRouteSidecarUnavailableError(OmniRouteSidecarError):
    def __init__(self, message: str) -> None:
        super().__init__(503, message, body=None)


class OmniRouteSidecarClient:
    ...
    @property
    def config(self) -> OmniRouteSidecarConfig:
        return self._config
    ...
    def _headers(self) -> dict[str, str]:
        headers = {
            ...
            "User-Agent": "codex-lb/omniroute-sidecar",
        }
    ...
    async def list_models(self) -> list[SidecarModel]:
        ...
        if not isinstance(raw_models, list):
            raise OmniRouteSidecarError(502, "Missing 'data' key in OmniRoute models response", body=data)
        ...
        for entry in raw_models:
            ...
            models.append(
                SidecarModel(
                    id=model_id,
                    created=created_at,
                    owned_by=owned_by if isinstance(owned_by, str) else "omniroute",
    ...
    async def list_models_cached(self) -> list[SidecarModel]:
        ...
        try:
            ...
            if self._models_cache is not None:
                logger.warning("using cached OmniRoute sidecar models after refresh failure", exc_info=True)
                return list(self._models_cache)
            logger.warning("OmniRoute sidecar models unavailable", exc_info=True)
            return []
    ...
    async def chat_completion(self, payload: Mapping[str, JsonValue]) -> JsonValue:
        ...
        try:
            ...
            raise OmniRouteSidecarUnavailableError(_transport_message(exc, "call OmniRoute sidecar")) from exc
    ...
    async def stream_chat_completion(self, payload: Mapping[str, JsonValue]) -> AsyncIterator[AsyncIterator[bytes]]:
        ...
        try:
            ...
            raise OmniRouteSidecarUnavailableError(_transport_message(exc, "stream OmniRoute sidecar")) from exc
...
def _error_from_status(status_code: int, body: JsonValue) -> OmniRouteSidecarError:
    message = f"OmniRoute sidecar returned HTTP {status_code}"
    ...
    return OmniRouteSidecarError(status_code, message, body=body)
