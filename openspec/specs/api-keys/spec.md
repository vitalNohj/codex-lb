...
When computing native API-key usage, request-log, reservation, or aggregate cost for `gpt-6-sol` or `gpt-6-luna`, the system MUST use these published USD-per-1M-token rates for input, cached input, and output. External integration costs remain governed by [external-model-pricing](../external-model-pricing/spec.md), not this native table:
