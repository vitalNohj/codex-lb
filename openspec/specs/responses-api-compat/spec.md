...
When replay fails or is declined, the terminal event MUST preserve a captured upstream code, message, and type rather than replace them with `stream_incomplete`. The shared pre-created replay boundary MUST preserve that error and its recovery metadata (`resets_in_seconds` and `resets_at`) across replay preparation and unsuccessful exits. A later specific upstream failure MAY supersede it together with its recovery metadata, but a generic retry or transport failure MUST NOT replace either. Successful replay MUST leave stale error overrides cleared. Explicit continuity masking remains governed by the continuity requirements.

Before HTTP output is committed, an upstream `usage_limit_reached` MUST surface as HTTP 429 with the upstream message. A genuine `no_accounts` selection failure remains HTTP 503. Capacity refusals MUST attach `Retry-After` from the available recovery hint, preferring the winning upstream error's recovery metadata over a local capacity retry interval. Reclassifying the same failure MUST preserve its recovery hint, but a subsequent selection failure MUST NOT inherit an earlier account's hint. The local interval is only a fallback when the winning error has no usable upstream hint. For HTTP 429 and 503, a positive finite `resets_in_seconds`, or the positive finite time remaining until `resets_at` when `resets_in_seconds` is absent, supplies a rounded-up `Retry-After`. JSON error rendering MUST retain an already supplied header. When neither upstream metadata nor a local recovery hint supplies a usable delay, no `Retry-After` is fabricated. After output is committed, the terminal error travels in-stream under the existing streaming contract. A transport drop with no captured upstream error retains the 502 `stream_incomplete` fallback.
