...
#### Scenario: Sidecar payload normalizes Cursor tool history to OpenAI chat format

- **GIVEN** `claude_sidecar_enabled=true`
- **WHEN** a client sends `POST /v1/chat/completions` whose `messages` include an assistant Cursor-native `tool_use` content part followed by a referenced user Cursor-native `tool_result` content part
- **THEN** the service forwards the assistant tool use as an OpenAI-compatible `tool_calls` entry
- **AND** the service forwards the user tool result as a `tool` role message with a matching `tool_call_id`
- **AND** the forwarded `messages` array does not include Cursor-native `tool_use` or `tool_result` content parts

#### Scenario: Sidecar payload normalizes Cursor tool definitions to OpenAI chat format

- **GIVEN** `claude_sidecar_enabled=true`
- **WHEN** a client sends `POST /v1/chat/completions` whose `tools` array includes Cursor-native flat tool definitions with `name`, `description`, and `input_schema`
- **THEN** the service forwards those tools as OpenAI-compatible `{"type":"function","function":...}` definitions
- **AND** each forwarded `function.parameters` value matches the client-provided `input_schema`
- **AND** the forwarded payload does not include Cursor-native flat tool definitions
