...
The service MUST support OpenCode Free full-model exact matches and prefix matches using the same full-model precedence, longest-prefix, and per-prefix strip rules as other sidecar integrations. OpenCode Free dispatch MUST apply only to `POST /v1/chat/completions` in this change and MUST NOT route `/v1/responses` requests to OpenCode Free. New installations MUST seed prefixes `oc/` and `opencode/` with strip enabled. OpenCode Free dispatch MUST succeed without an API key when the integration is enabled.
...
#### Scenario: Alternate opencode prefix strips wire model

- **GIVEN** the OpenCode Free sidecar is enabled
- **AND** OpenCode Free prefixes include `opencode/` with strip enabled
- **WHEN** a client sends `POST /v1/chat/completions` with `model: "opencode/deepseek-v4-flash-free"`
- **THEN** the service routes the request to OpenCode Free
- **AND** the forwarded payload uses `model: "deepseek-v4-flash-free"`
...
### Requirement: Apply sidecar effort override and DeepSeek V4 repair on both new chat paths

The service MUST apply the operator-configured sidecar reasoning-effort override as a forced value on OrcaRouter and OpenCode Free Chat Completions payloads, and it MUST run the existing DeepSeek V4 `reasoning_content` repair on both chat-completions paths.
