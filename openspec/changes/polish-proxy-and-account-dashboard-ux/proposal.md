...
- `frontend-architecture`: Add presentation/interaction requirements for (1) proxy-admin
  ...
  account-list add-account entry point that opens a chooser dialog (add via OAuth vs import), and
  (5) the account status filter sharing the "Need help?" row with a full-width search input.
...
- Frontend only (`frontend/`, built into `app/static`):
  ...
  - `frontend/src/features/accounts/components/account-detail.tsx` and list components (polish pass)
