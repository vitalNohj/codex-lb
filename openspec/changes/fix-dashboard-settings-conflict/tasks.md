...
- [x] 3.6 `get_fresh` commits a clean session then refreshes; dirty ORM returns the identity-map row (no nested session); sidecar origin version advances on save and stays pinned on failure; collection pre-flight skips only non-overlapping own writes.
