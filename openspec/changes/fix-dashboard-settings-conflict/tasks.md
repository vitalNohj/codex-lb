...
- [x] 3.6 `get_fresh` commits a clean session then refreshes; dirty ORM reads via a dedicated session; sidecar origin version advances on save and stays pinned on failure; collection pre-flight skips only non-overlapping own writes.
