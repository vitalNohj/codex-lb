...
If `/models` returns OpenRouter-shaped `pricing` objects, parse them into the runtime pricing registry under `provider="orcarouter"`. OrcaRouter and OpenRouter both list ids such as `deepseek/deepseek-chat` at different prices, so the ids must not share one key space. `app/core/usage/runtime_pricing.py` owns the registry's ownership, snapshot-replacement, and eviction rules; `list_models` here just has to keep feeding it a fully parsed listing and to raise (never record) on transport, HTTP, or response-shape failure, so a failed fetch is never mistaken for a delisting. Do not add invented rows to `DEFAULT_PRICING_MODELS`. Unknown models log `cost_usd = null`. Models ending in `-free` still go through `is_known_free_model`.
