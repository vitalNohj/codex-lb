...
1. Add `_OPUS_5_5_ID` beside the Fable 5.1 regex. Do not add `*claude-opus-5-5*` to `DEFAULT_MODEL_ALIASES`. The family glob stays for real Opus 5 ids, including date stamps.
...
5. One-shot migration appends `claude-opus-5-5` when absent. Case-insensitive presence, invalid JSON, and non-list values are left unchanged. Downgrade removes only that id. This is not re-applied on later settings saves, so an operator can remove the pin afterward.
