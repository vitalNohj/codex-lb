...
compacts correctly. Claude sidecar (cliproxyapi, `cp-*`) models are listed with
only `id`, `created`, `owned_by`, and `api_types` — no context window.
...
- Advertise an input context window for Claude sidecar models on
  `GET /v1/models`, mirroring the capability/context fields registry models
  already expose (`context_length`, `contextLength`,
  `capabilities.context_length`).
- Use the standard Claude window (200000) for sidecar `cp-*` models.
