...
A list saved through `PUT /api/claude-sidecar/routing/excluded-models` MUST be verified to govern CLIProxyAPI's own account selection, not only the request codex-lb sends. Verification of native selection behavior MUST exercise the real CLIProxyAPI parser, model registration, account manager, selector and retry paths, with a fake permitted only at the provider execution boundary. Verification of saved-file behavior MUST exercise actual file-watcher delivery through CLIProxyAPI's real runtime update consumer and same-Manager selection. The Python-to-Go HTTP boundary from codex-lb to CLIProxyAPI's real management handler and FileTokenStore MUST remain a separate cross-language contract and MUST NOT be inferred from native-library tests or a substituted sidecar client. Each verification establishes the contract only for the CLIProxyAPI version and routing configuration it exercises; it is not a claim about untested versions, strategies, or credential kinds. Entries MUST match the wire model ID literally: an exact entry covers only that ID, and only an explicitly configured wildcard covers a family. An alias MUST NOT restore a wire model excluded on the same account. The excluded account MUST NOT be selected on the first attempt or on any in-request retry, another eligible account MUST still be able to serve, and when no eligible account remains the existing exhaustion outcome MUST be preserved rather than falling back to an excluded account. Saving or clearing a list MUST change later selections in the already-running process, with no restart. Verification MUST use synthetic accounts and tokens, deny provider network access, and be skipped rather than silently weakened when its required confinement is unavailable.
