...
- After rewriting costs, add the newly computed Opus 5 / Sonnet 5 `cost_usd` onto existing `account_usage_rollups` / `api_key_usage_rollups` rows for folded logs (`requested_at <= folded_through`). Leave `folded_through` unchanged. Rewinding the watermark cannot rebuild request logs retention already deleted; those models folded as $0 so a cost-only delta is safe. Downgrade subtracts the same delta while costs are still present, then NULLs the request-log rows.
