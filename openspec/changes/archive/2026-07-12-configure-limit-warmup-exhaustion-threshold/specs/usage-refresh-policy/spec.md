...
#### Scenario: Warm-up is skipped below the exhausted threshold
- **GIVEN** limit warm-up is enabled globally and for an account
- **AND** the configured exhausted threshold is `99.0`
- **AND** the account's previous usage sample for a selected window was `98.9`
- **WHEN** background usage refresh records a newer sample for that window with `used_percent < 100` and a later `reset_at`
- **THEN** background usage refresh MUST NOT send warm-up traffic for that window
