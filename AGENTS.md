...
- Cursor IDE's manual `/summarize` button does not route through custom OpenAI-compatible providers or the configured proxy base URL — it connects directly to Cursor's internal infrastructure (api2.cursor.sh). The `/backend-api/codex/memories/trace_summarize` endpoint in codex-lb serves the Codex CLI's memory consolidation pipeline, not Cursor IDE's summarize button. These are separate mechanisms; auto-compact works via error matching on the proxy's chat-completions responses.
