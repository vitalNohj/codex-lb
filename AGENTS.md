...
- DeepSeek V4 thinking-mode tool calls fail in Cursor (400 "reasoning_content in the thinking mode must be passed back") because DeepSeek requires prior assistant `reasoning_content` echoed back with tool-call history and Cursor does not. The fix belongs in the existing OpenRouter/OmniRoute sidecar dispatch path for `/v1/chat/completions` (not native Codex routing); `ChatCompletionsRequest`'s payload dump already preserves `reasoning_content` inside `messages` on the sidecar path. Reference behavior: `yxlao/deepseek-cursor-proxy` (cache assistant `reasoning_content` and re-inject it into later tool-call turns; accumulate streaming deltas).
