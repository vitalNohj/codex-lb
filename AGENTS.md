...
- OpenSpec strict validation parses only the FIRST LINE of a requirement body as its description text when checking for MUST/SHALL; a requirement whose opening line lacks MUST/SHALL fails `--strict` even when MUST appears in later lines. Put a MUST/SHALL on the requirement's first sentence/line. Debug parsed deltas with `openspec change show <change> --json --deltas-only`.
- OpenSpec strict validation parses only the FIRST LINE of a requirement body as its description text when checking for MUST/SHALL; a requirement whose opening line lacks MUST/SHALL fails `--strict` even when MUST appears in later lines. Put a MUST/SHALL on the requirement's first sentence/line. Debug parsed deltas with `openspec change show <change> --json --deltas-only`.
